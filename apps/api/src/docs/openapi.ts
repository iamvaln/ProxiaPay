...
export const SERVERS: Record<ApiEnvironment, string> = {
  production: 'https://pay.proxia-digital.com/v1',
  sandbox: 'https://sandbox.pay.proxia-digital.com/v1',
};

/**
 * Builds the machine-readable interface specification of spec 7.5 from the request schemas the
 * implementation validates with, so the document describes the interface as it is. The API serves
 * it at /docs/openapi.json; `npm run openapi` writes it to a file.
 */
export function buildOpenApi(env: ApiEnvironment) {
